 ...
 */
interface Controller : Identifiable {
    suspend fun startControlling(): Boolean
    val devices: Set<Device>
    val identifieables: Set<Identifiable>  // ???
        get() = devices
}

sealed interface Device : Identifiable
 ...
 */
sealed interface Sensor : Device {
    suspend fun startSensing()
}

sealed interface SensorWithState<Value> : Sensor {
 ...
 */
sealed interface Actuator : Device
